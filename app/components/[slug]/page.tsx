    ...
    <main>
      ...
      <article className="detail-main" id="component-content">
        ...
        <header className="detail-hero">
          <div>
            <p className="eyebrow">COMPONENT {String(item.order).padStart(2, "0")} · {category?.en}</p>
            <h1>{item.name.zh}</h1>
        ...
        <section className="detail-section" aria-labelledby="demo-heading">
          <div className="detail-section-title">
            <div>
              <p className="eyebrow">TRY IT YOURSELF</p>
              <h2 id="demo-heading">交互式演示</h2>
            </div>
            <p>所有状态只保留在当前页面，不会上传任何数据。</p>
          </div>
          <InteractiveDetail slug={item.slug} />
        </section>

        <section className="detail-section" aria-labelledby="guide-heading">
          <div className="detail-section-title">
            <div>
        ...
        <section className="detail-section" aria-labelledby="prompt-heading">
          <div className="detail-section-title">
            <div>
              <p className="eyebrow">DESCRIBE IT TO AI</p>
              <h2 id="prompt-heading">这样告诉你的编程助手</h2>
            </div>
          </div>
          <CopyPrompt prompt={item.aiPrompt} />
        </section>

        <section className="detail-section" aria-labelledby="code-heading">
          <div className="detail-section-title">
            <div>
          ...
          <section className="detail-section" aria-labelledby="related-heading">
            ...
            <div className="related-grid">
              {related.map((relatedItem) => (
                <Link className="related-card" href={`/components/${relatedItem.slug}`} key={relatedItem.slug}>
